...
def export_locs(
    ...
    if "npz" in export_parameters["export_extension"]:
        ...
        np.savez(
            os.path.join(tracks_output_path, f"locs_{index:04}.npz"),
    ...
    if "hdf5" in export_parameters["export_extension"]:
        ...
        with h5py.File(
            os.path.join(tracks_output_path, f"locs_{index:04}.hdf5"), "w"
        ) as f:
    ...
    if "csv" in export_parameters["export_extension"]:
        ...
        export_csv(
            localizations,
            os.path.join(tracks_output_path, f"locs_{index:04}.csv"),
        )
...
def export_tracks(
    ...
    if "npz" in export_parameters["export_extension"]:
        tracks_output_path = os.path.join(export_parameters["folder_output"], "npz")
        np.savez(
            os.path.join(tracks_output_path, f"tracks_{index:04}.npz"),
    ...
    if "hdf5" in export_parameters["export_extension"]:
        tracks_output_path = os.path.join(export_parameters["folder_output"], "hdf5")
        with h5py.File(
            os.path.join(tracks_output_path, f"tracks_{index:04}.hdf5"), "w"
        ) as f:
    ...
    if "csv" in export_parameters["export_extension"]:
        ...
        export_csv(
            tracks[0],
            os.path.join(tracks_output_path, f"tracks_{index:04}_interp.csv"),
        )
        export_csv(
            tracks[1],
            os.path.join(tracks_output_path, f"tracks_{index:04}_raw.csv"),
        )
