"""This file contains the ULM_rfdetr class."""
...
class ULM:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.rfdetr_model_path = rfdetr_model_path
    ...
    def super_localization(
        ...
        mask, intensity = get_rfdetr_intensity_matrix(
            iq, 
    ...
    def create_tracks(self, localizations: np.ndarray) -> np.ndarray:
        ...
        frames_uniques = np.unique(localizations['frame_no'])
        faux_points = np.zeros(len(frames_uniques) * 2, dtype=localizations.dtype)
        
        for i, f in enumerate(frames_uniques):
            # Point 1 : Deep in negative space
            faux_points['pos'][2*i] = [-10000.0, -10000.0, -10000.0]
            faux_points['frame_no'][2*i] = f
            
            # Point 2 : Deep in positive space
            faux_points['pos'][2*i+1] = [10000.0, 10000.0, 10000.0]
            faux_points['frame_no'][2*i+1] = f
            
        # fusion
        locs_hack = np.concatenate([localizations, faux_points])
        ...
        try:
            ...
            tracked_loc = tracked_loc[tracked_loc['pos'][:, 0] > -5000]
            tracked_loc = tracked_loc[tracked_loc['pos'][:, 0] < 5000]
            
        except ValueError as e:
            if "zero-size array to reduction operation maximum" in str(e):
                logger.warning("Tracking interrupted: Not enough RFDETR data for this block.")
                raw_tracks_dtype = [("pos", float, 3), ("time", float), ("track_ind", int)]
                interp_tracks_dtype = [("pos", float, 3), ("velocity", float, 3), ("time", float), ("track_ind", int)]
                return [np.zeros(0, dtype=interp_tracks_dtype), np.zeros(0, dtype=raw_tracks_dtype)]
            else:
                raise e
        ...
        for track_id in track_ids:
            ...
            # extend() adds list elements without global memory reallocation
            all_raw_tracks.extend(raw_track)
            all_interp_tracks.extend(interp_track)

        # Final one-shot conversion (Zero-Copy overhead)
        raw_tracks_dtype = [
            ("pos", float, 3),
            ("time", float),
            ("track_ind", int),
...
def _gray_to_pil_rgb(img_gray: np.ndarray) -> Image.Image:
    """
    Convertit une image grayscale uint8 en image PIL RGB.
    """
    if img_gray.dtype != np.uint8:
        raise ValueError("L'image doit être en uint8 avant conversion PIL.")
    return Image.fromarray(img_gray).convert("RGB")


def _detections_to_xywh(detections) -> np.ndarray:
    """
    Convertit les détections RF-DETR (xyxy) en centres xywh.
    Retourne un tableau shape (N, 4): [cx, cy, w, h]
    """
...
def get_rfdetr_intensity_matrix(iq: np.ndarray, model_path: str, conf_thresh: float = 0.5) -> Tuple[np.ndarray, np.ndarray]:
    """
    Replaces maxima search with RFDETR inference on MIP projections.
    (Updated with volume edge protection)
    """
    from loguru import logger
    logger.info("Starting RFDETR detection via MIP projection...")
    
    try:
        rfdetr_model = RFDETRSmall(pretrain_weights=model_path)
        rfdetr_model.optimize_for_inference()
    except Exception as e:
        logger.error(f"Could not load RFDETR from {model_path} : {e}")
        return None, None

    mask = np.zeros(iq.shape, dtype=bool)
    nb_frames = iq.shape[3]
    ...
    for frame_idx in range(nb_frames):
        ...
        mip_face = np.max(vol_3d, axis=1)   
        mip_profil = np.max(vol_3d, axis=0) 

        img_face = cv2.normalize(mip_face, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
        img_profil = cv2.normalize(mip_profil, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)

        pil_face = _gray_to_pil_rgb(img_face)
        pil_profil = _gray_to_pil_rgb(img_profil)

        try:
            det_face = rfdetr_model.predict(pil_face, threshold=conf_thresh)
            det_profil = rfdetr_model.predict(pil_profil, threshold=conf_thresh)
        except Exception as e:
            logger.warning(f"Inference failed on frame {frame_idx}: {e}")
            continue

        boxes_face = _detections_to_xywh(det_face)
        boxes_profil = _detections_to_xywh(det_profil)
        
        for b_face in boxes_face:
            zf, xf, _, _ = b_face 
            
            for b_profil in boxes_profil:
                zp, yp, _, _ = b_profil 
                ...
                if abs(zf - zp) < 3.0: 
                    ...
                    if marge <= x_3d < (vol_3d.shape[0] - marge) and \
                       marge <= y_3d < (vol_3d.shape[1] - marge) and \
                       marge <= z_3d < (vol_3d.shape[2] - marge):
                        
                        mask[x_3d, y_3d, z_3d, frame_idx] = True
                        break

    intensity = mask * iq
    logger.info("3D RFDETR detection completed.")
...
def clean_and_interpolate_track(
    ...
    """
    Clean and interpolation function to process raw microbubble tracks.
    Performs smoothing and interpolation.

    Fonction modifié pour RF DETR car la méthode de détection détecte des tracks 
    plus courte, plus fragmenté ce qui engendre des erreurs 
    ...
    Returns:
        [raw_list, interp_list]
        or [[], []] if the track is too short / invalid
    """
    ...
    interp_step = interp_factor * np.min(scale[:3])

    if interp_step <= 0:
        raise ValueError("interp_factor * min(scale[:3]) must be > 0")

    ca_interp = np.arange(ca[0], ca[-1], interp_step)

    if len(ca_interp) < 2:
        ca_interp = np.array([ca[0], ca[-1]], dtype=float)

    pos_i = np.zeros((len(ca_interp), pos.shape[1]), dtype=float)
    for dim in range(pos.shape[1]):
        pos_i[:, dim] = np.interp(ca_interp, ca, pos[:, dim])

    pos_i = smooth(pos_i, scale[2] * 2)


    ca_i = get_curvilinear_abscissa(pos_i)

    if len(ca_i) < 2 or ca_i[-1] <= 0 or ca[-1] <= 0:
        raw_list = [
            (
                pos[i, :].T,
                index_frames[i],
