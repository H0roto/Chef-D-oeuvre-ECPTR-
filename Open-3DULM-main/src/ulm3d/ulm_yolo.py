"""This file contains the ULM_yolo class."""
...
class ULM:
    ...
    def super_localization(
        ...
        mask, intensity = get_yolo_intensity_matrix(
            iq, 
    ...
    def create_tracks(self, localizations: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            if "zero-size array to reduction operation maximum" in str(e):
                logger.warning("Tracking interrupted: Not enough YOLO data for this block.")
...
def get_yolo_intensity_matrix(iq: np.ndarray, model_path: str, conf_thresh: float = 0.5) -> Tuple[np.ndarray, np.ndarray]:
    """
    Replaces maxima search with YOLO inference on MIP projections.
    (Updated with volume edge protection)
    """
    from loguru import logger
    logger.info("Starting YOLO detection via MIP projection...")
    
    try:
        yolo_model = YOLO(model_path)
    except Exception as e:
        logger.error(f"Could not load YOLO from {model_path} : {e}")
        return None, None
    ...
    intensity = mask * iq
    logger.info("3D YOLO detection completed.")
